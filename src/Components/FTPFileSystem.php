...
class FTPFileSystem implements FileSystemInterface
{
    ...
    {
        throw new NotImplementedException('Method ' . __METHOD__ . ' not applicable for FTP file system.');
    }
    ...
    {
        throw new NotImplementedException('Updating container properties is not applicable for FTP storage.');
    }
    ...
    {
        throw new NotImplementedException('Updating folder properties is not applicable for FTP storage.');
    }
    ...
    public function streamFile($container, $path, $download = false)
    {
        try {
            ...
            $path = array_get($result, 'path');

            if (array_get($result, 'type') === 'file' && !empty($path)) {
                $file = basename($path);
                $meta = $this->adapter->getMetadata($path);
                ...
                Log::debug('Failed to stream FTP file: ' . $path);
    ...
    {
        throw new NotImplementedException('Updating file properties is not applicable for FTP storage.');
    }
